...
class DBService: NSObject {
    ...
    func getEntityList(type: String) -> [NSManagedObject]?{
        let entityDescription = NSEntityDescription.entityForName(type, inManagedObjectContext: managedObjectContext!)
        let request = NSFetchRequest()
        request.entity = entityDescription
        return managedObjectContext?.executeFetchRequest(request, error: nil) as? [NSManagedObject]
    }
    ...
    lazy var applicationDocumentsDirectory: NSURL = {
        // The directory the application uses to store the Core Data store file. This code uses a directory named "com.jcloutier.CoreData" in the application's documents Application Support directory.
